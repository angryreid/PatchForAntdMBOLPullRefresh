...
var PullToRefresh = function (_React$Component2) {
    ...
    function PullToRefresh() {
        ...
        _this2.isEdge = function (ele, direction) {
            console.log('nick, edge condition ...')
            var container = _this2.props.getScrollContainer();
            if (container && container === document.body) {
                // In chrome61 `document.body.scrollTop` is invalid
                var scrollNode = document.scrollingElement ? document.scrollingElement : document.body;
                if (direction === UP) {
                    console.log("PTR: Fixed btm pull refreshing.")
                    var height = Math.abs(scrollNode.scrollHeight - scrollNode.scrollTop - window.innerHeight)
            ...
            if (direction === UP) {
                console.log("PTR: Fixed btm pull refreshing.")
                var height = Math.abs(ele.scrollHeight - ele.scrollTop - ele.clientHeight)
                return height >=0 && height <= 1;
            }
            if (direction === DOWN) {
